...
function extractBasicAuth(authHeader: string | undefined): { apiKey: string; secretKey: string } | null {
...
export function authMiddleware(): MiddlewareHandler {
  return async (c, next) => {
    ...
    if (authHeader.startsWith('Basic ')) {
      ...
      const payload = await authenticateBasicCredential(deps.pool, credentials.apiKey, credentials.secretKey);
      if (!payload) {
        deps.metrics.authFailures.add(1, { scheme: 'basic' });
        throw new HttpError(401, 'Unauthorized');
      }
