...
const ISO_DURATION_RE = /^P(?:\d+W|(?:\d+Y)?(?:\d+M)?(?:\d+D)?(?:T(?:\d+H)?(?:\d+M)?(?:\d+(?:\.\d+)?S)?)?)$/;
...
const ACTOR_ALLOWED_KEYS = new Set(['objectType', 'name', 'mbox', 'mbox_sha1sum', 'openid', 'account', 'member']);
...
const RESULT_ALLOWED_KEYS = new Set(['score', 'success', 'completion', 'response', 'duration', 'extensions']);
...
function validateLanguageMap(map: unknown, path: string, errors: ValidationError[]): map is Record<string, string> {
  if (!isObject(map)) {
...
function validateAgent(obj: unknown, path: string, errors: ValidationError[]): obj is XAPIAgent {
  ...
  if (obj.objectType !== undefined && obj.objectType !== 'Agent') {
    errors.push({ path: `${path}.objectType`, message: 'Agent objectType must be "Agent" if present' });
    return false;
  }
  ...
  if (ifiCount === 0) {
    errors.push({ path, message: 'Agent must have exactly one IFI (mbox, mbox_sha1sum, openid, or account)' });
    return false;
  }
  if (ifiCount > 1) {
  ...
  if (obj.mbox !== undefined) {
    if (typeof obj.mbox !== 'string' || !obj.mbox.startsWith('mailto:') || obj.mbox.indexOf('@', 7) <= 7) {
      errors.push({
        path: `${path}.mbox`,
  ...
  if (obj.mbox_sha1sum !== undefined) {
    if (typeof obj.mbox_sha1sum !== 'string' || !/^[0-9a-f]{40}$/i.test(obj.mbox_sha1sum)) {
      errors.push({ path: `${path}.mbox_sha1sum`, message: 'mbox_sha1sum must be a 40-character hex string' });
      return false;
    }
  }
  ...
  if (obj.account !== undefined) {
    ...
    if (typeof obj.account.name !== 'string') {
      errors.push({ path: `${path}.account.name`, message: 'account.name is required and must be a string' });
      return false;
    }
  }
...
function validateGroup(obj: unknown, path: string, errors: ValidationError[]): obj is XAPIGroup {
  ...
  if (ifiCount === 0) {
    ...
    if (!Array.isArray(obj.member) || obj.member.length === 0) {
      errors.push({ path: `${path}.member`, message: 'Anonymous Group must have a non-empty member array' });
      return false;
    }
  } else if (ifiCount > 1) {
...
function validateInteractionComponent(
  ...
  if (typeof obj.id !== 'string') {
    errors.push({ path: `${path}.id`, message: 'Interaction component id is required and must be a string' });
    return false;
  }
...
function validateActivityDefinition(
  ...
  if (obj.description !== undefined) validateLanguageMap(obj.description, `${path}.description`, errors);

  if (obj.type !== undefined) {
    if (typeof obj.type !== 'string' || !isValidIRI(obj.type)) {
      errors.push({ path: `${path}.type`, message: 'Activity definition type must be a valid IRI' });
    }
  }
  ...
  if (obj.interactionType !== undefined) {
    if (typeof obj.interactionType !== 'string' || !VALID_INTERACTION_TYPES.has(obj.interactionType)) {
      errors.push({
  ...
  if (obj.correctResponsesPattern !== undefined) {
    if (!Array.isArray(obj.correctResponsesPattern)) {
      errors.push({ path: `${path}.correctResponsesPattern`, message: 'correctResponsesPattern must be an array' });
    } else {
  ...
  const INTERACTION_DEPENDENT_KEYS = ['correctResponsesPattern', 'choices', 'scale', 'source', 'target', 'steps'];
    ...
    if (obj[listKey] !== undefined) {
      if (!Array.isArray(obj[listKey])) {
        ...
        for (let i = 0; i < (obj[listKey] as unknown[]).length; i++) {
          validateInteractionComponent((obj[listKey] as unknown[])[i], `${path}.${listKey}[${i}]`, errors);
        }
      }
    }
  }
...
function validateExtensions(obj: unknown, path: string, errors: ValidationError[]): void {
  ...
  for (const key of Object.keys(obj)) {
    if (!isValidIRI(key)) {
      errors.push({ path: `${path}["${key}"]`, message: `Extension key "${key}" must be a valid IRI` });
    }
  }
}

function validateActivity(obj: Record<string, unknown>, path: string, errors: ValidationError[]): boolean {
...
function validateStatementRef(obj: Record<string, unknown>, path: string, errors: ValidationError[]): boolean {
...
function validateSubStatement(
  ...
  for (const forbidden of ['id', 'stored', 'version', 'authority']) {
    if (obj[forbidden] !== undefined) {
      errors.push({ path: `${path}.${forbidden}`, message: `SubStatement must not have "${forbidden}"` });
    }
  }
  ...
  if (obj.object === undefined) {
    ...
    if (isObject(obj.object) && obj.object.objectType === 'SubStatement') {
      errors.push({ path: `${path}.object`, message: 'SubStatement must not contain a nested SubStatement' });
    } else {
...
function validateObject(obj: unknown, path: string, errors: ValidationError[], insideSubStatement = false): boolean {
  if (!isObject(obj)) {
...
function validateResult(obj: unknown, path: string, errors: ValidationError[]): obj is XAPIResult {
  ...
  if (obj.duration !== undefined) {
    if (typeof obj.duration !== 'string' || !isValidISO8601Duration(obj.duration)) {
      errors.push({ path: `${path}.duration`, message: 'duration must be a valid ISO 8601 duration' });
    }
  }
  ...
  for (const key of ['parent', 'grouping', 'category', 'other'] as const) {
    if (obj[key] !== undefined) {
      ...
      if (Array.isArray(obj[key])) {
        ...
        errors.push({ path: `${path}.${key}`, message: `${key} must be an Activity or array of Activities` });
        continue;
      }
...
function validateContext(
  ...
  if (obj.revision !== undefined) {
    ...
    if (objType !== undefined && objType !== 'Activity') {
      errors.push({ path: `${path}.revision`, message: 'revision is only allowed when Object is an Activity' });
    }
  }
  if (obj.platform !== undefined) {
    ...
    if (objType !== undefined && objType !== 'Activity') {
      errors.push({ path: `${path}.platform`, message: 'platform is only allowed when Object is an Activity' });
    }
  }

  if (obj.language !== undefined) {
    if (typeof obj.language !== 'string' || !isValidLanguageTag(obj.language)) {
      errors.push({ path: `${path}.language`, message: 'language must be a valid RFC 5646 language tag' });
    }
  }
  ...
  if (obj.contextActivities !== undefined) {
    ...
    validateContextActivities(obj.contextActivities, `${path}.contextActivities`, errors, normalizedCA);
  ...
  if (obj.statement !== undefined) {
    if (!isObject(obj.statement) || (obj.statement as Record<string, unknown>).objectType !== 'StatementRef') {
...
function validateAttachments(obj: unknown, path: string, errors: ValidationError[]): obj is XAPIAttachmentMeta[] {
  if (!Array.isArray(obj)) {
  ...
  for (let i = 0; i < obj.length; i++) {
    ...
    if (typeof att.usageType !== 'string' || !isValidIRI(att.usageType)) {
      errors.push({ path: `${attPath}.usageType`, message: 'usageType is required and must be a valid IRI' });
    }
    ...
    if (typeof att.contentType !== 'string') {
      errors.push({ path: `${attPath}.contentType`, message: 'contentType is required and must be a string' });
    }
    if (typeof att.length !== 'number' || !Number.isInteger(att.length)) {
      errors.push({ path: `${attPath}.length`, message: 'length is required and must be an integer' });
    }
    if (typeof att.sha2 !== 'string') {
      errors.push({ path: `${attPath}.sha2`, message: 'sha2 is required and must be a string' });
    }
    if (att.fileUrl !== undefined && (typeof att.fileUrl !== 'string' || !isValidIRI(att.fileUrl))) {
...
export function validateStatement(input: unknown): ValidationResult {
  ...
  if (stmt.timestamp !== undefined) {
    if (typeof stmt.timestamp !== 'string' || !isValidISO8601Timestamp(stmt.timestamp)) {
      errors.push({ path: 'timestamp', message: 'timestamp must be a valid ISO 8601 timestamp with timezone' });
    }
  }
  ...
  if (stmt.version !== undefined) {
    if (typeof stmt.version !== 'string' || !/^1\.0(\.\d+)?$/.test(stmt.version)) {
      errors.push({ path: 'version', message: 'version must match 1.0.x (e.g. "1.0", "1.0.0", "1.0.3")' });
    }
  }
  ...
  if (stmt.authority !== undefined) {
    ...
    if (isObject(stmt.authority) && (stmt.authority as Record<string, unknown>).objectType === 'Group') {
      ...
      if (groupIfiCount > 0) {
        errors.push({ path: 'authority', message: 'Authority Group must be anonymous (no IFI on the group)' });
      }
      // Must have exactly 2 members
      if (!Array.isArray(authGroup.member) || authGroup.member.length !== 2) {
        errors.push({ path: 'authority.member', message: 'Authority Group must have exactly 2 members' });
      }
    }
  }
