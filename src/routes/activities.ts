...
export function createActivitiesApp() {
  ...
  app.openapi(putStateRoute, async (c) => {
    ...
    await withClient(pool, metrics, async (client) => {
      const existing = await getStateDocument(client, { stateId, activityIri: activityId, agentIfi, registration });
  ...
  app.openapi(deleteStateRoute, async (c) => {
    ...
    await withClient(pool, metrics, async (client) => {
      if (stateId) {
        ...
        await deleteStateDocument(client, { stateId, activityIri: activityId, agentIfi, registration });
      } else {
        await deleteAllStateDocuments(client, { activityIri: activityId, agentIfi, registration, since });
      }
    });
