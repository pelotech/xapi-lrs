...
export async function getStateDocument(
  client: PoolClient,
  params: { stateId: string; activityIri: string; agentIfi: string; registration: string | undefined },
...
export async function listStateIds(
  client: PoolClient,
  params: { activityIri: string; agentIfi: string; registration: string | undefined; since: string | undefined },
): Promise<string[]> {
  const result = await client.query({
    ...SELECT_STATE_IDS,
