...
export async function buildStatementEvent(
  ...
  return {
    ...
    id: row.statement_id,
    registrationId:
      (data.registration_id as string) ?? (stmtContext.registration as string) ?? null,
    sessionId:
      (data.session_id as string) ??
      (extensions["https://w3id.org/xapi/cmi5/context/extensions/sessionid"] as string) ??
      null,
    verbIri:
      String(data.verb_iri ?? "") ||
      (((row.payload.verb as Record<string, unknown>)?.id as string) ?? ""),
