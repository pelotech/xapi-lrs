...
export function canonicalAgentIfi(agent: string | Record<string, unknown>): string {
  ...
  const ifiCount =
    (agentObj.mbox ? 1 : 0) + (agentObj.mbox_sha1sum ? 1 : 0) + (agentObj.openid ? 1 : 0) + (agentObj.account ? 1 : 0);

  if (ifiCount !== 1) {
    throw new HttpError(400, 'Agent must have exactly one IFI (mbox, mbox_sha1sum, openid, or account)');
  }
