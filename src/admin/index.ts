...
import {
  verifyPassword, getDashboardCounts, getRecentStatements,
  listAccounts, createAccount, deleteAccount, changePassword,
  listCredentials, createCredential, deleteCredential, rotateSecret, setCredentialScopes,
  listAttachments, getAttachment,
  listStateDocuments, listActivityProfiles, listAgentProfiles,
  getStateDocumentById, getActivityProfileById, getAgentProfileById,
  deleteStateDocumentById, deleteActivityProfileById, deleteAgentProfileById,
...
import {
  documentsPage, stateDocumentTable, activityProfileTable, agentProfileTable,
  documentDetailView, bulkDeleteResult, deletedDocRow,
...
export function createAdminApp(deps: AdminDeps): Hono<AdminEnv> {
  ...
  function renderPage(c: Context<AdminEnv>, content: RawHtml) {
    ...
    return c.html(layout({ title: 'Admin', path: c.req.path, username: session.username, csrfToken: csrf }, content).value);
  }
  ...
  app.post('/accounts', async (c) => {
    ...
    deps.logger.info({ admin: session.username, action: 'account.create', target: username }, 'Admin account created');
  ...
  app.delete('/accounts/:id', async (c) => {
    ...
    deps.logger.info({ admin: session.username, action: 'account.delete', target: accountId }, 'Admin account deleted');
  ...
  app.put('/accounts/:id/password', async (c) => {
    ...
    deps.logger.info({ admin: session.username, action: 'account.changePassword', target: accountId }, 'Password changed');
  ...
  app.post('/credentials', async (c) => {
    ...
    deps.logger.info({ admin: session.username, action: 'credential.create', target: credId }, 'Credential created');
  ...
  app.delete('/credentials/:id', async (c) => {
    ...
    deps.logger.info({ admin: session.username, action: 'credential.delete', target: credId }, 'Credential deleted');
  ...
  app.post('/credentials/:id/rotate', async (c) => {
    ...
    deps.logger.info({ admin: session.username, action: 'credential.rotate', target: credId }, 'Secret rotated');
  ...
  app.put('/credentials/:id/scopes', async (c) => {
    ...
    await setCredentialScopes(deps.pool, deps.metrics, credId, scopes);
    deps.logger.info({ admin: session.username, action: 'credential.scopes', target: credId, scopes }, 'Scopes updated');
  ...
  app.post('/statements/:id/void', async (c) => {
    ...
    await withClient(deps.pool, deps.metrics, (client) =>
      voidStatement(client, statementId),
    );
  ...
  app.delete('/documents/state/:id', async (c) => {
    ...
    deps.logger.info({ admin: session.username, action: 'document.delete', target: c.req.param('id'), type: 'state' }, 'State document deleted');
  ...
  app.delete('/documents/activity-profile/:id', async (c) => {
    ...
    deps.logger.info({ admin: session.username, action: 'document.delete', target: c.req.param('id'), type: 'activity-profile' }, 'Activity profile deleted');
    return c.html(deletedDocRow().value);
  });
  ...
  app.delete('/documents/agent-profile/:id', async (c) => {
    ...
    deps.logger.info({ admin: session.username, action: 'document.delete', target: c.req.param('id'), type: 'agent-profile' }, 'Agent profile deleted');
    return c.html(deletedDocRow().value);
  });
  ...
  app.delete('/documents/state/bulk', async (c) => {
    ...
    deps.logger.info({ admin: session.username, action: 'document.bulkDelete', activityIri, agentIfi, count }, 'Bulk delete state documents');
