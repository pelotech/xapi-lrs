...
export function registerStatementRoutes(
  ...
  app.get('/statements/:id', async (c) => {
    ...
    const row = await withClient(deps.pool, deps.metrics, (client) => getStatementById(client, statementId));
  ...
  app.post('/statements/:id/void', async (c) => {
    ...
    c.var.logger.info({ admin: session.username, action: 'statement.void', target: statementId }, 'Statement voided');
