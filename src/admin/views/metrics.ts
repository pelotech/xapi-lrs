  ...
  for (const line of text.split("\n")) {
    if (line.startsWith("# HELP ")) {
      ...
      if (braceOpen !== -1) {
        ...
        current.values.push({
          labels: line.slice(braceOpen + 1, braceClose),
          value: line.slice(braceClose + 2),
        });
      } else {
...
const GROUPS: Record<string, string[]> = {
  ...
  "xAPI Operations": [
    "lrs_xapi_statements_received_total",
    "lrs_xapi_statements_stored_total",
    "lrs_xapi_statement_validation_errors_total",
    "lrs_xapi_document_operations_total",
  ],
