...
export async function rotateSecret(pool: Pool, metrics: LrsMetrics, credentialId: string, newSecret: string): Promise<void> {
  await poolQuery(pool, metrics, { ...ROTATE_SECRET, values: [newSecret, credentialId] });
}

export async function setCredentialScopes(pool: Pool, metrics: LrsMetrics, credentialId: string, scopes: string[]): Promise<void> {
...
export async function listStateDocuments(pool: Pool, metrics: LrsMetrics, limit: number, offset: number) {
  const [rows, count] = await Promise.all([
    poolQuery<StateDocumentListRow>(pool, metrics, { ...LIST_STATE_DOCUMENTS, values: [limit, offset] }),
...
export async function listActivityProfiles(pool: Pool, metrics: LrsMetrics, limit: number, offset: number) {
  const [rows, count] = await Promise.all([
    poolQuery<ActivityProfileListRow>(pool, metrics, { ...LIST_ACTIVITY_PROFILES, values: [limit, offset] }),
...
export async function listAgentProfiles(pool: Pool, metrics: LrsMetrics, limit: number, offset: number) {
  const [rows, count] = await Promise.all([
    poolQuery<AgentProfileListRow>(pool, metrics, { ...LIST_AGENT_PROFILES, values: [limit, offset] }),
    poolQuery<{ count: number }>(pool, metrics, COUNT_AGENT_PROFILES),
  ]);
  return { rows: rows.rows, total: count.rows[0]?.count ?? 0 };
}

export async function getStateDocumentById(pool: Pool, metrics: LrsMetrics, id: string): Promise<DocumentDetail | null> {
  const result = await poolQuery<DocumentDetail>(pool, metrics, { ...GET_STATE_DOCUMENT_BY_ID, values: [id] });
  return result.rows[0] ?? null;
}

export async function getActivityProfileById(pool: Pool, metrics: LrsMetrics, id: string): Promise<DocumentDetail | null> {
  const result = await poolQuery<DocumentDetail>(pool, metrics, { ...GET_ACTIVITY_PROFILE_BY_ID, values: [id] });
  return result.rows[0] ?? null;
}

export async function getAgentProfileById(pool: Pool, metrics: LrsMetrics, id: string): Promise<DocumentDetail | null> {
  const result = await poolQuery<DocumentDetail>(pool, metrics, { ...GET_AGENT_PROFILE_BY_ID, values: [id] });
  return result.rows[0] ?? null;
}
...
export async function bulkDeleteStateDocuments(pool: Pool, metrics: LrsMetrics, activityIri: string, agentIfi: string): Promise<number> {
  const result = await poolQuery(pool, metrics, { ...BULK_DELETE_STATE_DOCUMENTS, values: [activityIri, agentIfi] });
...
export async function listAttachments(pool: Pool, metrics: LrsMetrics, statementId: string): Promise<AttachmentListRow[]> {
  const result = await poolQuery<AttachmentListRow>(pool, metrics, { ...LIST_ATTACHMENTS, values: [statementId] });
  return result.rows;
}

export async function getAttachment(pool: Pool, metrics: LrsMetrics, statementId: string, sha: string): Promise<{ contents: Buffer; content_type: string } | null> {
  const result = await poolQuery<{ contents: Buffer; content_type: string }>(pool, metrics, { ...GET_ATTACHMENT, values: [statementId, sha] });
  return result.rows[0] ?? null;
}
...
export async function ensureAdminAccount(pool: Pool, metrics: LrsMetrics, username: string, password: string): Promise<void> {
