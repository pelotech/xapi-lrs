...
export async function bootstrapAccounts(
  ...
  if (config.apiKeyDefault && config.apiSecretDefault) {
    const account = bootstrapUsername
      ? await deps.getAccountByUsername(pool, metrics, bootstrapUsername)
      : null;
    if (account) {
      await deps.ensureDefaultCredential(
        pool,
        metrics,
        config.apiKeyDefault,
        config.apiSecretDefault,
        account.id,
      );
