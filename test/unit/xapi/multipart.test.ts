...
function buildMultipartBody(
  boundary: string,
  parts: { headers: string; body: string | Buffer }[],
): Buffer {
  const chunks: Buffer[] = [];
...
describe("buildMultipartResponse", () => {
  ...
  it("JSON part comes first, attachment parts follow", async () => {
    ...
    const resp = await buildMultipartResponse({ id: "1" }, [
      { sha2: "hash1", contentType: "application/pdf", stream },
    ]);
  ...
  it("attachment parts have correct headers", async () => {
    ...
    const resp = await buildMultipartResponse({}, [
      { sha2: "myhash", contentType: "image/png", stream },
    ]);
    const text = await resp.text();
