...
describe('xAPI Activity Profile Resource', () => {
  ...
  describe('GET /xapi/activities/profile', () => {
    ...
    test('should return list of profileIds when profileId is omitted', async ({ server, authToken }) => {
      ...
      await fetch(`${server.apiUrl}/xapi/activities/profile?${profileParams({ profileId: 'alpha' })}`, {
        method: 'PUT',
        headers,
        body: JSON.stringify({ a: 1 }),
      });
      await fetch(`${server.apiUrl}/xapi/activities/profile?${profileParams({ profileId: 'beta' })}`, {
        method: 'PUT',
        headers,
    ...
    test('should filter profileId list by since parameter', async ({ server, authToken }) => {
      ...
      await fetch(`${server.apiUrl}/xapi/activities/profile?${profileParams({ profileId: 'old-prof' })}`, {
        method: 'PUT',
        headers,
      ...
      await fetch(`${server.apiUrl}/xapi/activities/profile?${profileParams({ profileId: 'new-prof' })}`, {
        method: 'PUT',
        headers,
        body: JSON.stringify({ new: true }),
      });

      const resp = await fetch(`${server.apiUrl}/xapi/activities/profile?${profileParams({ since: midpoint })}`, {
        headers: { Authorization: `Bearer ${authToken}`, 'X-Experience-API-Version': '1.0.3' },
      });
