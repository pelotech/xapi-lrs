...
async function queryStates(
  ...
  return result.rows.map(
    (r: { state_id: string; contents: Buffer; content_type: string; last_modified: Date }) => ({
      state_id: r.state_id,
      state_data: r.content_type.includes("application/json")
        ? JSON.parse(r.contents.toString("utf8"))
        : r.contents,
...
describe("xAPI Activity State Resource", () => {
  ...
  describe("PUT /xapi/activities/state", () => {
    test("should store a new state document and return 204", async ({
      pool,
      server,
      basicAuth,
    }) => {
      ...
      const response = await fetch(
        `${server.apiUrl}/xapi/activities/state?${stateParams({ stateId: "bookmark" })}`,
        {
          method: "PUT",
          headers: {
    ...
    test("should store state with registration context", async ({ pool, server, basicAuth }) => {
      ...
      const result = await pool.query(
        `SELECT registration FROM state_document WHERE state_id = $1`,
        ["reg-state"],
      );
  ...
  describe("POST /xapi/activities/state (merge)", () => {
    test("should create state document when none exists", async ({ pool, server, basicAuth }) => {
      const response = await fetch(
        `${server.apiUrl}/xapi/activities/state?${stateParams({ stateId: "merge-test" })}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
    ...
    test("should merge top-level keys into existing document", async ({
      pool,
      server,
      basicAuth,
    }) => {
    ...
    test("should not deep-merge nested objects (top-level only)", async ({
      pool,
      server,
      basicAuth,
    }) => {
  ...
  describe("GET /xapi/activities/state", () => {
    test("should return single state document when stateId is provided", async ({
      server,
      basicAuth,
    }) => {
      // Store a state
      ...
      // Get it back
      const response = await fetch(
        `${server.apiUrl}/xapi/activities/state?${stateParams({ stateId: "doc-1" })}`,
        {
          headers: { Authorization: `Basic ${basicAuth}`, ...V },
        },
      );
    ...
    test("should return list of stateIds when stateId is omitted", async ({
      server,
      basicAuth,
    }) => {
      const headers = {
    ...
    test("should filter stateId list by since parameter", async ({ server, basicAuth }) => {
      ...
      // Store first state
      await fetch(
        `${server.apiUrl}/xapi/activities/state?${stateParams({ stateId: "old-state" })}`,
        {
          method: "PUT",
          headers,
      ...
      // Store second state
      await fetch(
        `${server.apiUrl}/xapi/activities/state?${stateParams({ stateId: "new-state" })}`,
        {
          method: "PUT",
          headers,
      ...
      const response = await fetch(
        `${server.apiUrl}/xapi/activities/state?${stateParams({ since: midpoint })}`,
        {
          headers: { Authorization: `Basic ${basicAuth}`, ...V },
        },
      );
  ...
  describe("DELETE /xapi/activities/state", () => {
    test("should delete a single state document by stateId", async ({
      pool,
      server,
      basicAuth,
    }) => {
      ...
      // Store
      await fetch(
        `${server.apiUrl}/xapi/activities/state?${stateParams({ stateId: "to-delete" })}`,
        {
          method: "PUT",
          headers,
    ...
    test("should delete all state documents when stateId is omitted", async ({
      pool,
      server,
      basicAuth,
    }) => {
    ...
    test("should respect since parameter when deleting all", async ({
      pool,
      server,
      basicAuth,
    }) => {
      ...
      // Store second state
      await fetch(
        `${server.apiUrl}/xapi/activities/state?${stateParams({ stateId: "delete-me" })}`,
        {
          method: "PUT",
          headers,
