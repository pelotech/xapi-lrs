...
describe("GET /xapi/agents — Person Object", () => {
  ...
  test("agent with no stored statements falls back to input echo", async ({
    server,
    basicAuth,
  }) => {
  ...
  test("Person Object only includes IFI types present in stored statements", async ({
    server,
    basicAuth,
  }) => {
    ...
    await postStatements(server.apiUrl, basicAuth, [
      makeStatement({ actor: { mbox, name: "Dana" } }),
    ]);
