...
describe("POST generic xAPI statements", () => {
  ...
  test("accepts statement without objectType (defaults to Activity)", async ({
    server,
    basicAuth,
  }) => {
    const resp = await postStatement(
  ...
  test("GET returns stored, authority, and server-generated fields", async ({
    server,
    basicAuth,
  }) => {
    const id = randomUUID();
...
describe("ADL conformance test before-hook statement", () => {
  test("accepts the before-hook statement that was previously rejected", async ({
    server,
    basicAuth,
  }) => {
    const resp = await postStatement(server.apiUrl, basicAuth, {
