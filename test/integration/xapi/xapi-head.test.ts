...
describe('xAPI HEAD Support', () => {
  ...
  test('HEAD /xapi/statements returns same status as GET with empty body', async ({ server, authToken }) => {
  ...
  test('HEAD /xapi/statements includes X-Experience-API-Consistent-Through', async ({ server, authToken }) => {
  ...
  test('HEAD /xapi/statements returns 400 without version header', async ({ server, authToken }) => {
    const resp = await fetch(`${server.apiUrl}/xapi/statements`, {
      method: 'HEAD',
