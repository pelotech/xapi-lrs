...
async function generateSigningKey(
  alg: string = "RS256",
): Promise<{ privateKey: CryptoKey; privateKeyPem: string }> {
...
describe("xAPI Signed Statements — structural validation", () => {
  ...
  describe("POST /xapi/statements — invalid signed statements", () => {
    ...
    test("should reject signed statement with corrupted JSON payload", async ({
      server,
      basicAuth,
    }) => {
      const stmt = minimalStatement();
    ...
    test("should reject signed statement with invalid JWS binary", async ({
      server,
      basicAuth,
    }) => {
